...
class TokenHandler
{
    ...
    public static function getTokenWithPassword(string $username, string $password, OAuthConfig $config, $loginListener): string
    {
        try {
            ...
            $tokenResponse = makeApiCall(UrlEndpointUtils::createTokenUrl($config->getBaseUrl()), $headers, $postData);
            session_start();
            ...
            $_SESSION['id_token'] = $tokenResponse["id_token"] ?? null;
    ...
    public static function validateToken($token, OAuthConfig $config, $tokenTypeHint = ''): bool
    {
        ...
        $response = makeApiCall($validationUrl, $headers, $postData);
    ...
    public static function getNewAccessToken(string $refreshToken, OAuthConfig $config): array
    {
        ...
        // Use the makeApiCall function to make the POST request
        $response = makeApiCall($tokenUrl, $headers, $postData);
